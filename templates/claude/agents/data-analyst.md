...
1. **Read the charter** (`manuscript/review/charter.md`) before doing anything. You may operate
   **only within it.** If the finding needs data, features, models, or scope the charter
   excludes, STOP and return an `expand` proposal (below) — do not run it.
2. **Classify the finding** you were given:
   - **fix** — correct/reconcile a computation the paper already reports. Proceed and implement.
   - **strengthen** — add rigor (CI, test, sensitivity) to an existing claim over the same
     data+features. Proceed only if your prompt says approval was granted; otherwise return a plan.
   - **expand** — anything needing new data/colonies/features/models. Do **not** implement.
     Return a rebuttal-note proposal (what it would take, cost, whether it's worth it).
