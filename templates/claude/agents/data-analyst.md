...
3. Re-run the unit(s) and **pass the gate**: `paper-forge gate` must succeed (strict compile +
   numeric-literal guard + verdict-claim guard + frozen-verdict guard + citation guard + check-rqs). Run `pytest -q` if the project has unit tests.
   You edit result units, so the frozen-verdict guard is the one that will catch you: a
   verdict string a unit *states* (`interp = "The effect was significant."`) is flagged;
   one it *derives* by branching on the statistic is not.
