...
`interpretations.yaml` and reference it as `{{interp.<output_key>}}`. Result units
...
6. **Result units emit numbers, not prose.** No "significant"/"not significant" strings
   in a result unit; that decision belongs in `interpretations.yaml`.
7. **Always run `make check`** after modifying the template to catch missing placeholders,
   hardcoded numbers, and unbacked verdicts.
8. **Always run `make compile`** after modifying result units to update the manuscript.
9. **Test result units** with `make test` before compiling.
10. **One JSON per result unit** — keep analyses modular and focused.
